...
def _reise_stationen_full():
    """Alle Etappen mit vollen Feldern aus SR_ROUTE in index.html geparst —
    eine Quelle, kein Drift (Frontend-JS bleibt die einzige Datenquelle)."""
    import re
    ...
    out = []
    for m in re.finditer(r"\{\s*nr:\s*\d+.*?\]\s*\}", src, re.S):
        b = m.group(0)
        ...
        out.append({
            ...
            "name": f(r"\bname:'([^']*)'"),
            "region": f(r"\bregion:'([^']*)'"),
            "farbe": f(r"\bfarbe:'([^']*)'", "#6366f1"),
            "lat": float(f(r"\blat:\s*([\d.]+)", "0") or 0),
            "lon": float(f(r"\blon:\s*([\d.]+)", "0") or 0),
            "tag": f(r"\btag:'([^']*)'"),
            "datum": f(r"\bdatum:'([^']*)'"),
            "tageszeit": f(r"\btageszeit:'([^']*)'"),
            "aufenthalt": aufenthalt,
            "anfahrt": f(r"\banfahrt:'([^']*)'"),
            "tipp": f(r"\btipp:'([^']*)'"),
            "beschreibung": f(r"\bbeschreibung:'([^']*)'"),
            "infoUrl": f(r"\binfoUrl:'([^']*)'"),
            "distKm": float(f(r"\bdistKm:\s*([\d.]+)", "0") or 0),
            "dauerMin": float(f(r"\bdauerMin:\s*([\d.]+)", "0") or 0),
...
def _reise_etappen():
    """Etappen (id → name, lat, lon) aus SR_ROUTE — schlanker Blick auf _reise_stationen_full()."""
    return {s["id"]: {"name": s["name"], "lat": s["lat"], "lon": s["lon"]}
            for s in _reise_stationen_full()}
...
BOARD_FILE = os.path.join(WORKSPACE, "data/board.json")
...
# ── Ladeplaner: Ladesäulen ENTLANG einer Route ───────────────────────────────
# Chris will keine "in der Nähe"-Liste, sondern: Ziel/Route eingeben → alle Säulen
...
# Von diesem Netz aus ist NUR overpass-api.de erreichbar; kumi/private.coffee laufen in Timeouts
# und würden jede fehlgeschlagene Abschnitts-Query um 2×35 s verzögern → daher raus.
_OVERPASS_MIRRORS = [
    "https://overpass-api.de/api/interpreter",
]

def _num(v):
    """Erste ganze Zahl aus einem OSM-Tag ziehen ('150 kW' → 150, '22.0' → 22). 0 wenn keine."""
    import re
    m = re.search(r'\d+', str(v if v is not None else ''))
    return int(m.group()) if m else 0

def _overpass(query, timeout=35, start_mirror=0):
    """Overpass-Abfrage. Primär overpass-api.de mit Backoff-Retry (gegen 429/Überlast),
    dann die Notnagel-Mirrors je einmal. Raise bei totalem Fehlschlag."""
    import urllib.request as _ureq, urllib.parse as _uparse, time as _t
    data = _uparse.urlencode({'data': query}).encode()
    prim = _OVERPASS_MIRRORS[0]
    # (URL, Wartezeit VOR diesem Versuch) — primär bis zu 3× mit wachsendem Backoff
    plan = [(prim, 0), (prim, 2.0), (prim, 4.0)] + [(m, 0) for m in _OVERPASS_MIRRORS[1:]]
    last = None
    for url, backoff in plan:
        if backoff:
            _t.sleep(backoff)
        try:
            req = _ureq.Request(url, data=data,
                                headers={'User-Agent': 'BollaMC/1.0 (chrismandel13@gmail.com)', 'Accept': 'application/json'})
            with _ureq.urlopen(req, timeout=timeout) as resp:
                return json.loads(resp.read().decode())
        except Exception as e:
            last = e
            continue
    raise last if last else RuntimeError("Overpass nicht erreichbar")

def _laden_route_geometry(lat1, lon1, lat2, lon2):
    """OSRM: volle Route-Geometrie (Liste [lat,lon]) + Gesamt-km + Dauer-min. None bei Fehler."""
    import urllib.request
    try:
        url = (f"http://router.project-osrm.org/route/v1/driving/"
               f"{lon1},{lat1};{lon2},{lat2}?overview=full&geometries=geojson")
        j = json.loads(urllib.request.urlopen(urllib.request.Request(url, headers=_REISE_UA), timeout=25).read())
        if j.get("code") == "Ok" and j.get("routes"):
            rt = j["routes"][0]
            coords = [(c[1], c[0]) for c in rt["geometry"]["coordinates"]]   # geojson = lon,lat
            return coords, round(rt["distance"]/1000, 1), round(rt["duration"]/60)
    except Exception:
        pass
    return None

def get_chargers_route(ziel, start_lat=None, start_lon=None, start=None, corridor_m=2000):
    """Ladesäulen entlang der Fahrroute Start→Ziel.
    Rückgabe: gruppiert in enbw / aral / sonstige, je Station mit km-Position auf der Route."""
    # Start bestimmen: explizite Koordinaten > Ortsname > Zuhause
    ...
    geo = _laden_route_geometry(s[0], s[1], zc[0], zc[1])
    if not geo:
        return {"error": "Route konnte nicht berechnet werden (OSRM nicht erreichbar).", "enbw": [], "aral": [], "sonstige": []}
    coords, dist_km, dauer_min = geo

    # Dichte Polyline (~ alle 2.5 km) für exakte Projektion Station→Route
    dec = [coords[0]]; cum = [0.0]; prev = coords[0]; running = 0.0; since = 0.0
    for c in coords[1:]:
        d = _laden_hav(prev[0], prev[1], c[0], c[1]); running += d; since += d; prev = c
        if since >= 2.5:
            dec.append(c); cum.append(running); since = 0.0
    if dec[-1] != coords[-1]:
        dec.append(coords[-1]); cum.append(running)

    # Route in ~150-km-Abschnitte zerlegen; je Abschnitt EINE leichte Overpass-Query
    # (Stützpunkte nur ~ alle 3 km — der "around"-Linestring folgt den Verbindungssegmenten).
    CHUNK_KM = 180.0
    chunks = []
    cur = [dec[0]]; chunk_start = 0.0; last_km = 0.0
    for i in range(1, len(dec)):
        if cum[i] - last_km >= 3.0 or i == len(dec) - 1:
            cur.append(dec[i]); last_km = cum[i]
        if cum[i] - chunk_start >= CHUNK_KM:
            chunks.append(cur); cur = [dec[i]]; chunk_start = cum[i]
    if len(cur) >= 2:
        chunks.append(cur)
    chunks = chunks[:12]   # Deckel (~1800 km)

    import time as _t
    elements = []; seen_ids = set(); errs = 0
    for ci, pts in enumerate(chunks):
        line = ",".join(f"{la},{lo}" for la, lo in pts)
        query = (f'[out:json][timeout:30];'
                 f'node["amenity"="charging_station"](around:{int(corridor_m)},{line});'
                 f'out body;')
        try:
            data = _overpass(query, timeout=35, start_mirror=ci)
            for el in data.get('elements', []):
                if el.get('id') in seen_ids:
                    continue
                seen_ids.add(el.get('id')); elements.append(el)
        except Exception:
            errs += 1
        if ci < len(chunks) - 1:
            _t.sleep(0.3)
    if not elements and errs:
        return {"error": "Ladesäulen-Suche gerade überlastet — bitte in ein paar Sekunden nochmal.", "enbw": [], "aral": [], "sonstige": []}
    ...
    def tier(op):
        ...
        for names, t, label, color in TIER_MAP:
            if any(n in o for n in names):
                return t, label, color
        return 3, '⚙️ Sonstige Anbieter', '#8e8e93'

    import math
    def _pt_seg(plat, plon, alat, alon, blat, blon):
        """Abstand (m) Punkt→Strecke A-B + Fraktion t entlang A-B (equirect. Näherung)."""
        latref = math.radians(plat)
        def xy(la, lo):
            return (math.radians(lo - plon) * math.cos(latref) * 6371000.0,
                    math.radians(la - plat) * 6371000.0)
    ...
    for el in data.get('elements', []):
        tags = el.get('tags', {})
        op = tags.get('operator') or tags.get('brand') or tags.get('network') or tags.get('name') or 'Unbekannt'
        name = tags.get('name') or op
        la, lo = el['lat'], el['lon']
        key = (round(la, 4), round(lo, 4), (name or '').lower()[:20])
        if key in seen:
            continue
        seen.add(key)
        # nächste Strecke der Polyline → exakter Korridorabstand + km-Position auf Route
        best_d, best_km = 1e12, 0.0
        for i in range(len(dec) - 1):
            d_m, t = _pt_seg(la, lo, dec[i][0], dec[i][1], dec[i+1][0], dec[i+1][1])
            if d_m < best_d:
                best_d = d_m
                best_km = cum[i] + t * (cum[i+1] - cum[i])
        if best_d > corridor_m * 1.3:
            continue
        addr = ' '.join(filter(None, [tags.get('addr:street',''), tags.get('addr:housenumber',''), tags.get('addr:city','')]))
        kw = _num(tags.get('maxpower:ev') or tags.get('charge') or tags.get('socket:type2_combo:output') or tags.get('socket:chademo:output'))
        t, tlabel, tcolor = tier(op)
        rows.append({
            'name': name, 'operator': op, 'addr': addr.strip(),
            'lat': la, 'lon': lo, 'kw': kw,
            'capacity': _num(tags.get('capacity')),
            'route_km': round(best_km, 1),           # km ab Start entlang der Route
            'detour_m': round(best_d),               # Abstand zum Fahrweg (m)
            'tier': t, 'tier_label': tlabel, 'tier_color': tcolor,
...
class Handler(BaseHTTPRequestHandler):
    ...
    def _handle_share(self, raw):
        ...
        Windows-Zwischenablage — damit Chris am Studio direkt Strg+V drücken kann."""
        import subprocess as _sp, uuid as _uuid, base64
        from email.parser import BytesParser
        from email.policy import default as _emaildefault

        ct = self.headers.get("Content-Type", "")
        if "multipart/form-data" not in ct.lower():
            self._send_share_page(False, "Kein multipart-Upload"); return

        msg = BytesParser(policy=_emaildefault).parsebytes(
            b"Content-Type: " + ct.encode() + b"\r\n\r\n" + raw)

        img_bytes, img_ext, text_field = None, ".jpg", ""
        for part in msg.iter_parts():
            ...
            if fname and payload:
                ...
                if e in (".jpg", ".jpeg", ".png", ".webp", ".gif", ".bmp", ".heic"):
                    img_ext = ".jpg" if e in (".jpg", ".jpeg") else e
                img_bytes = payload
            elif disp_name in ("text", "url", "title") and payload:
                try: text_field = (text_field + " " + payload.decode("utf-8", "ignore")).strip()
                except Exception: pass

        if not img_bytes:
            # Nur Text/Link geteilt → ins mmp-Clipboard hängen
            if text_field:
                try:
                    _clip = os.path.join(WORKSPACE, "config/clipboard.json")
                    entries = []
                    if os.path.isfile(_clip):
                        with open(_clip) as f: entries = json.load(f)
                    entries.insert(0, {"text": text_field, "source": "handy-share",
                                       "ts": datetime.now().isoformat()})
                    with open(_clip, "w") as f: json.dump(entries[:100], f, ensure_ascii=False)
                    self._send_share_page(True, "Text/Link ins mmp-Clipboard gelegt.", text=True); return
                except Exception as e:
                    self._send_share_page(False, f"Text-Ablage fehlgeschlagen: {e}"); return
            self._send_share_page(False, "Kein Foto empfangen"); return

        # Foto in OneDrive-Ordner speichern (Historie + Windows-erreichbarer Pfad)
        clipdir = "/mnt/d/OneDrive/Bilder/Handy-Clip"
        try:
            os.makedirs(clipdir, exist_ok=True)
        except Exception:
            clipdir = os.path.join(WORKSPACE, "config/clipboard_images"); os.makedirs(clipdir, exist_ok=True)
        fname = f"handy_{datetime.now().strftime('%Y%m%d_%H%M%S')}_{_uuid.uuid4().hex[:4]}{img_ext}"
        fpath = os.path.join(clipdir, fname)
        with open(fpath, "wb") as f:
            f.write(img_bytes)

        # WSL-Pfad → Windows-Pfad
        try:
            winpath = _sp.run(["wslpath", "-w", fpath], capture_output=True, text=True, timeout=10).stdout.strip()
        except Exception:
            winpath = ""
        ...
        if clip_ok:
            self._send_share_page(True, "Foto ist am Studio in der Zwischenablage — jetzt Strg+V drücken.")
        else:
            self._send_share_page(True, f"Foto gespeichert unter Bilder\\Handy-Clip\\{fname}, aber Zwischenablage klappte nicht (Studio gesperrt?).")

    def _send_share_page(self, ok, msg, text=False):
        icon = "✅" if ok else "⚠️"
        color = "#34c759" if ok else "#ff9500"
        html = f"""<!doctype html><html lang="de"><head><meta charset="utf-8">
<meta name="viewport" content="width=device-width,initial-scale=1">
<title>An MMP geteilt</title>
<style>body{{font-family:-apple-system,system-ui,sans-serif;background:#f2f2f7;margin:0;
display:flex;align-items:center;justify-content:center;min-height:100vh;padding:24px}}
.card{{background:#fff;border-radius:20px;padding:32px 28px;max-width:420px;text-align:center;
box-shadow:0 8px 30px rgba(0,0,0,.12)}}.ic{{font-size:56px}}h1{{font-size:20px;margin:14px 0 8px}}
p{{font-size:16px;color:#333;line-height:1.5;margin:0 0 22px}}
a{{display:inline-block;background:{color};color:#fff;text-decoration:none;font-size:17px;
font-weight:600;padding:13px 26px;border-radius:12px}}</style></head>
<body><div class="card"><div class="ic">{icon}</div>
<h1>{'Geteilt an MMP' if ok else 'Nicht geklappt'}</h1>
<p>{msg}</p><a href="/m">Zurück zu mmp</a></div></body></html>"""
    ...
    def do_GET(self):
        try:
            ...
            elif self.path.startswith("/api/laden/route"):
                q = urllib.parse.parse_qs(urllib.parse.urlparse(self.path).query)
                ziel = (q.get("ziel", [""])[0]).strip()
                if not ziel:
                    self._send_json({"error": "Kein Ziel angegeben.", "enbw": [], "aral": [], "sonstige": []}, status=400)
                else:
                    slat = q.get("start_lat", [None])[0]
    ...
    def do_POST(self):
        ...
        if self.path == "/api/share":
            try:
                self._handle_share(raw)
            except Exception as e:
                tb = traceback.format_exc()
                print(f"share error: {tb}")
                try:
                    self._send_share_page(False, str(e))
                except Exception:
                    pass
            return
...
{{
  ...
  "img_prompt": "[specific scene that powerfully visualizes the topic: concrete subject, action, environment, lighting], photorealistic, cinematic, detailed — no text or signage"
            ...
            elif self.path == "/api/reise/sightseeing-docx":
                teil = (body.get("teil") or "gesamt").strip()
                titel = body.get("titel") or "🧭 Sightseeing-Guide"
                subtitle = body.get("subtitle") or ""
                alle = _reise_stationen_full()
                if teil == "schwarzwald":
                    stationen = [s for s in alle if s["region"] == "Schwarzwald"]
                elif teil == "bodensee":
                    stationen = [s for s in alle if s["region"] != "Schwarzwald"]
                else:
                    stationen = alle
                docx_bytes = reise_sightseeing_docx(stationen, titel, subtitle)
                self.send_response(200)
                self.send_header("Content-Type", "application/vnd.openxmlformats-officedocument.wordprocessingml.document")
                self.send_header("Content-Disposition", f'attachment; filename="Sightseeing-Guide-{teil}.docx"')
                self.send_header("Content-Length", str(len(docx_bytes)))
                self.end_headers()
                self.wfile.write(docx_bytes)
...
# ── Reiseplaner: Sightseeing-Guide (nur Tipps & Highlights, für unterwegs) ────

def _reise_highlight_clean_name(highlight):
    """Emoji/Symbole vorn + Klammer-Zusatz hinten abschneiden, für Geocoding & Anzeige-Titel."""
    import re
    txt = re.sub(r'^[\W_]+', '', highlight, flags=re.UNICODE).strip()
    txt = re.sub(r'\s*\([^)]*\)\s*$', '', txt).strip()
    return txt or highlight


def _reise_geocode_highlight(highlight, station_name):
    """Versucht, einen Highlight-Namen auf eine Koordinate zu geocoden (Nominatim, kostenlos,
    gecacht). None bei Fehlschlag — Highlight bleibt dann ohne Platz auf der Laufplan-Karte."""
    clean = _reise_highlight_clean_name(highlight)
    for q in (f"{clean}, {station_name}, Deutschland", f"{clean}, {station_name}"):
        r = _reise_geocode(q)
        if r:
            return r
    return None
...
def _reise_haversine_m(lat1, lon1, lat2, lon2):
    import math
    R = 6371000
...
def _reise_walk_order(start_lat, start_lon, points):
    """Nearest-Neighbor-Reihenfolge ab Startpunkt — reicht für die Handvoll Stopps pro Stadt."""
    remaining = points[:]
    order = []
    cur = (start_lat, start_lon)
    while remaining:
        remaining.sort(key=lambda p: (p["lat"] - cur[0]) ** 2 + (p["lon"] - cur[1]) ** 2)
        nxt = remaining.pop(0)
        order.append(nxt)
        cur = (nxt["lat"], nxt["lon"])
    return order
...
def _reise_walk_map(name, points, farbe="#6366f1"):
    """Nummerierte Laufplan-Karte (Stil wie Flensburg-Rundgang) — ein Marker je Stopp in Laufreihenfolge."""
    from staticmap import StaticMap, CircleMarker
    from staticmap.staticmap import _lon_to_x, _lat_to_y
    ...
    m = StaticMap(900, 700, url_template="https://tile.openstreetmap.org/{z}/{x}/{y}.png")
    for p in points:
        m.add_marker(CircleMarker((p["lon"], p["lat"]), "#ffffff", 2))
    img = m.render(zoom=15)
    draw = ImageDraw.Draw(img)
    try:
        font_nr = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 19)
        font_hd = ImageFont.truetype("/usr/share/fonts/truetype/dejavu/DejaVuSans-Bold.ttf", 15)
    except Exception:
        font_nr = font_hd = ImageFont.load_default()
    rgb = hex2rgb(farbe)
    for i, p in enumerate(points, 1):
        tx, ty = _lon_to_x(p["lon"], m.zoom), _lat_to_y(p["lat"], m.zoom)
        px, py = m._x_to_px(tx), m._y_to_px(ty)
        r = 18
        draw.ellipse([px-r-2, py-r-2, px+r+2, py+r+2], fill=(10, 10, 10))
        draw.ellipse([px-r, py-r, px+r, py+r], fill=rgb)
        txt = str(i); bb = draw.textbbox((0, 0), txt, font=font_nr)
        draw.text((px-(bb[2]-bb[0])//2-bb[0], py-(bb[3]-bb[1])//2-bb[1]), txt, font=font_nr, fill="white")
    box_w = min(320, 40 + len(name) * 9)
    draw.rectangle([8, 8, 8 + box_w, 36], fill=(255, 255, 255, 230), outline=(160, 160, 160))
    draw.text((14, 13), f"LAUFPLAN {name.upper()}", font=font_hd, fill=(20, 20, 20))
    tf = tempfile.NamedTemporaryFile(suffix=".png", delete=False)
    img.save(tf.name, quality=92)
    return tf.name


def _reise_highlight_infos(station):
    """2-3 kurze, praktische Stichpunkte je Highlight für Orte mit längerem Aufenthalt.
    Claude fasst dabei nur den bereits kuratierten beschreibung/tipp-Text zusammen und ergänzt
    gesichertes Allgemeinwissen — KEINE Preise/Öffnungszeiten erfinden (die veralten/stimmen oft nicht).
    Bei Fehler: leeres Dict, Highlights bleiben ohne Zusatztext."""
    import shutil
    claude_bin = shutil.which("claude") or os.path.expanduser("~/.local/bin/claude")
    highlights_txt = "\n".join(f"- {h}" for h in station["highlights"])
    prompt = f"""Basierend NUR auf diesem bereits recherchierten Reiseführer-Text über {station['name']}:

{station['beschreibung']}
{station.get('tipp', '')}

Schreibe für jeden der folgenden Programmpunkte 2 bis 3 kurze, praktische Stichpunkte
(je 6-14 Wörter, Deutsch) — z.B. warum es sich lohnt, guter Zeitpunkt, was man dort erlebt.
Erfinde KEINE konkreten Preise oder Öffnungszeiten, stütze dich nur auf den obigen Text und
gesichertes Allgemeinwissen über {station['name']}:

{highlights_txt}

Gib NUR dieses JSON zurück (Highlight-Text als Key, exakt wie oben, Value = Liste von 2-3 Stichpunkten):
{{"infos": {{"<highlight1>": ["<punkt1>", "<punkt2>"], "<highlight2>": ["<punkt1>", "<punkt2>", "<punkt3>"]}}}}"""
    try:
        ...
        print(f"Highlight-Infos fehlgeschlagen ({station['name']}): {e}")
        return {}


def _keep_block_together(paragraphs):
    """Verhindert, dass eine Station mitten im Text auf die nächste Seite umbricht: jeder
    Absatz bleibt in sich zusammen (keep_together) und mit dem jeweils nächsten verkettet
    (keep_with_next) — Word schiebt den ganzen Block auf die nächste Seite statt ihn zu zerreißen."""
