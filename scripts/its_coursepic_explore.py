#!/usr/bin/env python3
"""Explore how course pictures work in itslearning - one-off exploration script."""
import json
import os
...
with sync_playwright() as pw:
    ...
    try:
        # 7b II course home
        its.goto("https://moin.itslearning.com/main.aspx?CourseID=190743", timeout=20000)
        its.wait_for_timeout(2000)
        its.screenshot(path=f"{OUT_DIR}/7bII_home.png", full_page=True)
        print("7bII home URL:", its.url)

        # Look for course settings / course picture link
        # Try common itslearning course admin paths
        html = its.content()
        with open(f"{OUT_DIR}/7bII_home.html", "w") as f:
            f.write(html)

        # Search for "Innstillinger" / "Einstellungen" / "Kursbild" links
        for kw in ["Einstellungen", "Kursbild", "Kursinformation", "Om kurset", "Course picture", "Kurs bearbeiten", "Redigera"]:
            cnt = its.get_by_text(kw, exact=False).count()
            if cnt:
                print(f"Found '{kw}': {cnt} matches")

        # Print visible nav tabs
        tabs = its.locator("nav a, [role=tab], .itsl-course-menu a").all_text_contents()
        print("Nav/tabs texts:", [t.strip() for t in tabs if t.strip()][:40])
