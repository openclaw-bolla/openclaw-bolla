...
def vorspann_xhtml(text):
    # use the vorspann from the JSON source (fallback to hardcoded EN constant)
    text = (text or "").strip()
    parts = text.split('\n', 1)
...
def chapter_xhtml(kapitel_obj, idx):
    ...
    # remove title-duplication: first text line if it repeats the chapter title (possibly **marked**)
...
<container version="1.0" xmlns="urn:oasis:names:tc:opendocument:xmlns:container">
  <rootfiles>
    <rootfile full-path="OEBPS/content.opf"
        ...
        z.writestr("OEBPS/Text/ueberautor.xhtml", backmatter_xhtml("About the Author", ueber_autor))
        z.writestr("OEBPS/Text/danksagung.xhtml", backmatter_xhtml("Acknowledgments", danksagung))
        z.writestr("OEBPS/Text/rezension.xhtml",  backmatter_xhtml("A Note on Reviews", rezensions_bitte))
