#!/usr/bin/env python3
"""Verifiziert nach dem Ersetzen: Download-Inhalt der Ressource in 7a I enthaelt den neuen Absatz,
und der bestehende Mitteilungs-Anhang-Link ist weiterhin intakt (kein 'Ressource nicht gefunden')."""
import os
...
with sync_playwright() as p:
    ...
    for fr in its.frames:
        ...
        if cand.count() > 0 and cand.first.is_visible():
            ...
            break
    if dl_btn is None:
        raise RuntimeError("Kein 'Herunterladen'-Button in irgendeinem Frame gefunden")
    print("Download-Button gefunden in Frame:", dl_frame.url)
    with its.expect_download(timeout=10000) as dl_info:
        dl_btn.click(timeout=5000)
    dl = dl_info.value
    save_path = f"{OUT_DIR}/downloaded_after_replace.html"
    dl.save_as(save_path)
    content = open(save_path, encoding="utf-8", errors="replace").read()
    print("Downloaded Groesse:", len(content))
    marker = "Microsoft-Anmeldeseite"
    print("Enthaelt neuen Hinweis-Absatz:", marker in content)

    # Announcement-Verifikation: Kurs-Uebersicht oeffnen, letzte Mitteilung mit Anhang pruefen
    its.goto(f"https://moin.itslearning.com/main.aspx?CourseID={COURSE_ID}", timeout=20000)
    its.wait_for_timeout(2000)
    its.screenshot(path=f"{OUT_DIR}/verify_course_overview.png", full_page=True)
